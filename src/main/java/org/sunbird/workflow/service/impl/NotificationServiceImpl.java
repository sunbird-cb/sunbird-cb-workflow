		...
		if (!ObjectUtils.isEmpty(wfStatus.getNotificationEnable()) && wfStatus.getNotificationEnable()) {
			...
			if (updatedFieldValue.isPresent()) {
				if (Constants.BLENDED_PROGRAM_SERVICE_NAME.equalsIgnoreCase(wfRequest.getServiceName())) {
					...
					params.put("body", configuration.getMailBody().replace(STATE_NAME_TAG, wfStatusEntity.getCurrentStatus())
							.replace(FIELD_KEY_TAG, toValue.entrySet().iterator().next().getKey()).replace(COMMENT_TAG, wfRequest.getComment() ).replace(TO_VALUE_TAG, (String) toValue.entrySet().iterator().next().getValue()));
