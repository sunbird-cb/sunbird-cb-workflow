...
public interface WfStatusRepo extends JpaRepository<WfStatusEntity, String> {
    ...
    @Query(value = "select update_field_values, wf_id, comment, lastupdated_on, current_status from wingspan.wf_status where service_name = ?1 and userid = ?3 order by lastupdated_on desc" , nativeQuery = true)
